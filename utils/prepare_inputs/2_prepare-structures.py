...
def main():
    ...
    # settings
    path_to_annotation_csv = "1_annotated_rcsb/failed_18.04.csv"
    option = "modeller"
    n_parallel = 18
    ...
    for pdb_id in pdb_ids:
        input_dir = f"{pdb_id}_{option}"
        data_dir = Path(run_dir) / "input" / input_dir
        out_dir = Path(run_dir) / "amber-topology" / f'{input_dir}'
        small_molecules_dir = Path(run_dir) / "small-molecules"
